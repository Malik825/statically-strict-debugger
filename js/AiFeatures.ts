...
class AIFeatures {
  ...
  private recognition: any | null = null; // SpeechRecognition type

  constructor(lightController: Light, wifiController: WifiController) {
  ...
  updatePredictiveSchedules(): void {
    ...
    components.forEach((comp: ComponentData) => {
      if (!comp.usage || comp.usage.length === 0) return;
      ...
      this.lightController.displayNotification(
        `${comp.name} schedule updated: On at ${comp.autoOn}, Off at ${comp.autoOff}`,
        'beforeend',
        document.body
      );
    });
  ...
  private processVoiceCommand(command: string): void {
    ...
    if (command.includes('all lights')) {
      ...
      if (turnOn || turnOff) {
        ...
        const generalIcon = document.querySelector('.general_light_switch img') as HTMLImageElement | null;
        if (generalIcon) {
        ...
        this.lightController.displayNotification(
          `All lights turned ${turnOn ? 'on' : 'off'}`,
          'beforeend',
          document.body
        );
        return;
      ...
      if (command.includes(roomName)) {
        ...
        if (turnOn || turnOff) {
          ...
          if (comp.isLightOn !== turnOn) {
            ...
            this.lightController.toggleLightSwitch(comp.element);
            this.lightController.displayNotification(
              `${comp.name} light turned ${turnOn ? 'on' : 'off'}`,
              'beforeend',
              document.body
            );
          }
    ...
    components.forEach((comp: ComponentData) => {
      ...
      if (avgUsage > 15) { // High usage threshold
        ...
        if (lateNightUsage && comp.isLightOn) {
          this.lightController.displayNotification(
            `High usage detected for ${comp.name} during late hours. Consider turning off or adjusting schedule.`,
            'beforeend',
            document.body
          );
        }
