...
class Light {
    ...
    toggleLightSwitch(element) {
        ...
        const roomElement = element.closest('.rooms');
        ...
        const lightSwitchImg = element.querySelector('.light-switch img');
        ...
        this.displayNotification(`${component.name} light turned ${component.isLightOn ? 'on' : 'off'}`, 'beforeend', document.body);
    }
    handleLightIntensitySlider(slider, value) {
        ...
        const roomElement = slider.closest('.rooms');
        if (!roomElement) {
        ...
        if (!component.isLightOn) {
            ...
            const lightSwitch = roomElement.querySelector('.light-switch');
            if (lightSwitch) {
                const lightSwitchImg = lightSwitch.querySelector('img');
                if (lightSwitchImg) {
                    const lightOnSrc = lightSwitchImg.dataset.lighton || './assets/svgs/light_bulb.svg';
        ...
        slider.value = value;
        const intensityValueSpan = slider.nextElementSibling;
        if (intensityValueSpan) {
            intensityValueSpan.textContent = value;
        }
        this.displayNotification(`${component.name} light intensity set to ${value}`, 'beforeend', document.body);
    }
